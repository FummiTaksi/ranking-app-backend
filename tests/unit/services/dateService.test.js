...
describe('dateService', () => {
  describe('isFall', () => {
    test('returns true when date is in july', () => {
      expect(dateService.isFall(new Date(2020, 7, 20))).toBeTruthy();
    });
    test('returns false when date is in june', () => {
      expect(dateService.isFall(new Date(2020, 6, 20))).toBeFalsy();
    });
  });

  describe('getFallAndSpringYears', () => {
    test('returns correct when date is in july', () => {
      const string = dateService.getFallAndSpringYears(new Date(2020, 7, 20));
